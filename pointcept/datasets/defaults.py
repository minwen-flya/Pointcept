...
class DefaultDataset(Dataset):
    ...
    def prepare_test_data(self, idx):
        ...
        data_dict_list = []
        if len(self.aug_transform) > 1:
            for aug in self.aug_transform:
                data_dict_list.append(aug(deepcopy(data_dict)))
                # data_dict_list = aug(deepcopy(data_dict))
        else:
            data_dict_list = self.aug_transform[0](deepcopy(data_dict))
        ...
        if self.test_crop is not None:
            ...
            for d in data_dict_list:
                cropped_list.extend(self.test_crop(d))
            data_dict_list = cropped_list
            del cropped_list
        fragment_list = []
        for data in data_dict_list:
            if self.test_voxelize is not None:
                data_part_list = self.test_voxelize(data)  # "index" is subset of the length(local grid_coord)
                if self.test_cfg.voxelize.mode == "train":
        ...
        result_dict["fragment_list"] = fragment_list  # ['coord', 'grid_coord', 'index', 'offset', 'feat', 'global_index']
        return result_dict
...
class FlyaDataset(Dataset):
    ...
    def __init__(
        ...
        super(FlyaDataset, self).__init__()
        self.data_root = data_root
        self.split = split
        self.transform = Compose(transform)
        self.cache = cache
    ...
    def prepare_test_data(self, idx):
        ...
        for i in range(len(fragment_list)):
            fragment_list[i] = self.post_transform(fragment_list[i])
        result_dict["fragment_list"] = fragment_list  # ['coord', 'grid_coord', 'index', 'offset', 'feat']
        return result_dict
